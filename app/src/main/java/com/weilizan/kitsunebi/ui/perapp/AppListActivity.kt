...
class AppListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        thread(start = true) {
            ...
            when (perAppMode) {
                0 -> {
                    val allowedAppList = Preferences.getString(applicationContext, getString(R.string.per_app_allowed_app_list), "")
                    selectedList.addAll(ArrayList(allowedAppList!!.split(",")))
                }
                1 -> {
                    val disallowedAppList = Preferences.getString(applicationContext, getString(R.string.per_app_disallowed_app_list), "")
